...
*******************************************************/
static class GameOfLife extends LXPattern {
  ...
  public GameOfLife(LX lx) {
    super(lx);
    this.lx.tempo.setBpm(80);
    this.state = new CellState[lx.total];
    this.newState = new CellState[lx.total];
    this.addModulator(
        this.hCenter = new SinLFO(modelSize * .25, modelSize * .75,
            modelSize * 1000)).trigger();
    this.addModulator(
        this.sPos = new SawLFO(-Math.max(modelSize, modelSize), modelSize
            + modelSize, modelSize * 300)).trigger();
    this.spawn();
  ...
  private int neighborsAlive(int i) {
    int x = i % this.modelSize;
    int y = i / this.modelSize;
  ...
  public void run(double deltaMs) {
    ...
    for (int i = 0; i < 400; ++i) {
      ...
      this.colors[i] = LXColor.hsb(
          (this.lx.getBaseHue() + (i)/modelSize * 2. + Math.abs((i)/modelSize
              - this.hCenter.getValue()) * 0.4) % 360,
          Math.min(
              100,
              30. + Math.abs(i / this.modelSize - this.sPos.getValue() + i
                  % this.modelSize) * 10.), b);
    }
  }
}